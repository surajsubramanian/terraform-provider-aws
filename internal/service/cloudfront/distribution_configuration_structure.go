...
func genAddressList(items []string) []*string {
...
func genValuesList(items []*string) []string {
	ret := make([]string, 0, len(items))
	for _, item := range items {
		ret = append(ret, *item)
	}
...
func expandTrustedKeyGroups(s []interface{}) *awstypes.TrustedKeyGroups {
	...
	if len(s) > 0 {
		...
		tkg.Items = genValuesList(flex.ExpandStringList(s))
...
func flattenTrustedKeyGroups(tkg *awstypes.TrustedKeyGroups) []interface{} {
	if tkg.Items != nil {
		return flex.FlattenStringList(genAddressList(tkg.Items))
	}
	return []interface{}{}
}
...
func ExpandTrustedSigners(s []interface{}) *awstypes.TrustedSigners {
	...
	if len(s) > 0 {
		...
		ts.Items = genValuesList(flex.ExpandStringList(s))
...
func FlattenTrustedSigners(ts *awstypes.TrustedSigners) []interface{} {
	if ts.Items != nil {
		return flex.FlattenStringList(genAddressList(ts.Items))
	}
	return []interface{}{}
}
...
func ExpandHeaders(d []interface{}) *awstypes.Headers {
	return &awstypes.Headers{
		...
		Items:    genValuesList(flex.ExpandStringList(d)),
	}
}

func FlattenHeaders(h *awstypes.Headers) []interface{} {
	if h.Items != nil {
		return flex.FlattenStringList(genAddressList(h.Items))
	}
	return []interface{}{}
}
...
func ExpandQueryStringCacheKeys(d []interface{}) *awstypes.QueryStringCacheKeys {
	items := make([]string, 0)
	for _, item := range flex.ExpandStringList(d) {
...
func FlattenQueryStringCacheKeys(k *awstypes.QueryStringCacheKeys) []interface{} {
	...
	if k.Items != nil {
		return flex.FlattenStringList(items)
	}
	return []interface{}{}
}
...
func ExpandAliases(s *schema.Set) *awstypes.Aliases {
	...
	if s.Len() > 0 {
		aliases.Items = genValuesList(flex.ExpandStringSet(s))
	}
	return &aliases
}
...
func FlattenAliases(aliases *awstypes.Aliases) *schema.Set {
	if aliases.Items != nil {
		return flex.FlattenStringSet(genAddressList(aliases.Items))
	}
...
func ExpandGeoRestriction(m map[string]interface{}) *awstypes.GeoRestriction {
	...
	if v, ok := m["locations"]; ok {
		gr.Items = genValuesList(flex.ExpandStringSet(v.(*schema.Set)))
...
func FlattenGeoRestriction(gr *awstypes.GeoRestriction) map[string]interface{} {
	...
	if gr.Items != nil {
		m["locations"] = flex.FlattenStringSet(genAddressList(gr.Items))
	}
	return m
}
...
func flattenKGKeyPairIds(keyPairIds []*awstypes.KGKeyPairIds) []interface{} {
	...
	for _, keyPairId := range keyPairIds {
		m := map[string]interface{}{
			...
			"key_pair_ids": aws.ToStringSlice(genAddressList(keyPairId.KeyPairIds.Items)),
		}
...
func flattenSigners(signers []*awstypes.Signer) []interface{} {
	...
	for _, signer := range signers {
		m := map[string]interface{}{
			...
			"key_pair_ids":       aws.ToStringSlice(genAddressList(signer.KeyPairIds.Items)),
		}
